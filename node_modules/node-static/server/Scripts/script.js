  ...
  this.removeOffer = function () {
    $.each(offers, function (i) {
      if (offers[i].id == offerId) {
        removedObjects[0].offers.push(offers[i]);
        offers.splice(i, 1);
        return false;
      }
    })
  ...
  function removeMessage(subject, $messageElem) {
    ...
    $.each(comments, function (i) {
      if (comments[i].id == commentId) {
        removedObjects[0][subject].push(comments[i]);
        offers[offerId][subject][0].count -= 1;
        comments.splice(i, 1);
        return false;
      }
...
Handlebars.registerHelper('renderImg', function (context, options) {
  var
    res = '<img class="offer-img" src="' + context + '">',
    width = $(res)[0].width;

  if (width < 200) {
    res = '<img class="offer-img small" src="' + context + '">'
  }
