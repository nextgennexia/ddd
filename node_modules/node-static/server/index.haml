%html
	...
	%body
		...
		.main-wrap.clearfix
			.login-wrap
				.author
					%img.avatar{:src => "images/users/anonymous.png"}
					.content
						%span Вы вошли как:
						%br
...
%script#social-info-templ{:type => "text/x-handlebars-template"}
	{{#renderCountActions likes.0.count}}
	%span.item.count-likes {{this}} нравится
	{{/renderCountActions}}
	{{#renderCountActions comments.0.count}}
...
%script#lightbox-social-info-templ{:type => "text/x-handlebars-template"}
	{{#if saved.0.count}}
	.saved-info
		{{#renderCountActions saved.0.count}}
		%span.title {{this}} добавили себе
...
%script#latest-comments-templ{:type => "text/x-handlebars-template"}
	{{#renderLatestMessages comments.0.fromUsers "comments"}}
	.author{:"data-id" => "{{id}}"}
		%img.avatar{:src => "{{avatar}}"}
		.content
...
%script#latest-answers-templ{:type => "text/x-handlebars-template"}
	{{#renderLatestMessages answers.0.fromUsers "answers"}}
	.author{:"data-id" => "{{id}}"}
		%img.avatar{:src => "{{avatar}}"}
		.content
...
%script#social-buttons-templ{:type => "text/x-handlebars-template"}
	{{#save saved.0.fromUsers}}
	%div{:class => "item fa fa-lightbulb-o {{saved}}", :"data-button" => "save"}
	{{/save}}
	{{#like likes.0.fromUsers}}
...
%script#lightbox-social-buttons-templ{:type => "text/x-handlebars-template"}
	{{#save saved.0.fromUsers}}
	%div{:class => "button {{saved}}", :"data-button" => "save"} Добавить к себе
	{{/save}}
	{{#like likes.0.fromUsers}}
